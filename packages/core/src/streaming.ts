/**
 * Streaming utilities for paginated API responses
 *
 ...
 * ```
 */
export const paginatedStream = <T, E extends MovieApiErrors>(
  fetchPage: (page: number) => Effect.Effect<PaginatedResponse<T>, E, never>,
  options: PaginationOptions = {},
): Stream.Stream<T, E, never> => {
  ...
  const pageStream: Stream.Stream<readonly T[], E, never> = Stream.paginateEffect(
  ...
  const flattened: Stream.Stream<T, E, never> = pageStream.pipe(
 ...
 *
 * Warning: This loads all results into memory. Use with caution for
 ...
 * ```
 */
export const collectAllPages = <T, E extends MovieApiErrors>(
  fetchPage: (page: number) => Effect.Effect<PaginatedResponse<T>, E, never>,
  options: PaginationOptions = {},
): Effect.Effect<readonly T[], E, never> =>
 ...
 * ```
 */
export const mapPaginated = <T, R, E extends MovieApiErrors>(
  fetchPage: (page: number) => Effect.Effect<PaginatedResponse<T>, E, never>,
  fn: (item: T) => R,
  options: PaginationOptions = {},
): Effect.Effect<readonly R[], E, never> =>
  paginatedStream(fetchPage, options).pipe(
    Stream.map(fn),
    Stream.runCollect,
    Effect.map((chunk) => Array.from(chunk)),
 ...
 * ```
 */
export const mapPaginatedEffect = <T, R, E extends MovieApiErrors, E2, Req>(
  fetchPage: (page: number) => Effect.Effect<PaginatedResponse<T>, E, never>,
  fn: (item: T) => Effect.Effect<R, E2, Req>,
  options: PaginationOptions & { readonly concurrency?: number } = {},
): Effect.Effect<readonly R[], E | E2, Req> => {
