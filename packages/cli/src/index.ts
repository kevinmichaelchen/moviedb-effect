...
import { Console, Effect, Layer, Config, ConfigProvider } from 'effect'
import { Movie, Person, Search, Tv, MovieDbClient, MovieDbConfig, RateLimiterLive } from 'moviedb-effect'
...
const ConfigLayer = Layer.effect(
  MovieDbConfig,
  Effect.gen(function* () {
    const apiKey = yield* Config.string('MOVIEDB_API_KEY')
    return {
      apiKey,
...
const personCommand = Command.make('person', { id: personId }, ({ id }) =>
  Effect.gen(function* () {
    ...
    yield* Console.log(
      `Biography: ${details.biography?.slice(0, 300) ?? 'No biography available'}${(details.biography?.length ?? 0) > 300 ? '...' : ''}`,
    )
  }).pipe(Effect.provide(AppLayer), Effect.scoped),
)
...
// Main moviedb command
const moviedb = Command.make('moviedb', {}, () => Console.log('MovieDB CLI - Use --help for available commands'))

// Combine all commands
const command = moviedb.pipe(Command.withSubcommands([searchCommand, movieCommand, personCommand, tvCommand]))
